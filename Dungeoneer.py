...
class Enemy:
    ...
    def Collide(self, x: float, y: float) -> bool:
        ...
        return (
            (self.collisionMap[x][y] in [Tile.EMPTY, Tile.CHEST])
            if 0 <= x < len(self.collisionMap) and 0 <= y < len(self.collisionMap[0])
            else False
        )
...
class Map:
    ...
    def Generate(self, enemyCount: int, bandageCount: int):
        self.map = [
            [Tile(Tile.EMPTY) for _ in range(MAP_SIZE)] for _ in range(MAP_SIZE)
        ]
        ...
        for i in range(len(self.rooms) - 1):
            ...
            while x0 != x1:
                ...
                if rand.random() < HALLWAY_ROOM_CHANCE / 2:
                    hallway_room_width = rand.randint(
                        HALLWAY_ROOM_MIN_SIZE, HALLWAY_ROOM_MAX_SIZE
                    )
                    hallway_room_height = rand.randint(
                        HALLWAY_ROOM_MIN_SIZE, HALLWAY_ROOM_MAX_SIZE
                    )
        ...
        self.startPos = (
            round(self.rooms[0][0] + self.rooms[0][2] / 2) + 0.5,
            round(self.rooms[0][1] + self.rooms[0][3] / 2) + 0.5,
        )
        self.endPos = (
            round(
                self.rooms[len(self.rooms) - 1][0]
                + self.rooms[len(self.rooms) - 1][2] / 2
            )
            + 0.5,
            round(
                self.rooms[len(self.rooms) - 1][1]
                + self.rooms[len(self.rooms) - 1][3] / 2
            )
            + 0.5,
        )
        ...
        # entrance
        self.map[round(self.rooms[0][0] + self.rooms[0][2] / 2)][
            round(self.rooms[0][1] + self.rooms[0][3] / 2)
        ].type = Tile.ENTRANCE
        # exit
        self.map[
            round(
        ...
        self.bandages = rand.sample(
            floor_positions, min(bandageCount, len(floor_positions))
        )
    ...
    def GetTile(self, x: int, y: int) -> Tile:
        return (
            self.map[x][y]
            if len(self.map) > x >= 0 and len(self.map[0]) > y >= 0
    ...
    def GenerateMapRenderMap(
        self, playerX: float, playerY: float
        ...
        for x in range(-1, MAP_SIZE + 1):
            for y in range(-1, MAP_SIZE + 1):
                ...
                if tile == Tile.EMPTY:
                    ...
                    if (
                        self.GetTile(x + 1, y + 1).hasFloor()
                        and self.GetTile(x + 1, y) == Tile.EMPTY
                        and self.GetTile(x, y + 1) == Tile.EMPTY
                    ):
        ...
        for x in range(-1, MAP_SIZE + 1):
            for y in range(-1, MAP_SIZE + 1):
                ...
                if tile.hasFloor():
                    ...
                    if self.GetTile(x, y - 1) == Tile.EMPTY:
                        left = (
                            self.GetTile(x + 1, y).hasFloor()
                            and self.GetTile(x + 1, y - 1) == Tile.EMPTY
                        )
                    ...
                    if tile == Tile.CHEST:
                        self.renderMap.append(
                            (
                                "closedchest" if not self.chestState else "emptychest",
                                x,
                                y,
                            )
                        )
                ...
                if tile == Tile.EMPTY:
                    ...
                    # Inner corner
                    if (
                        self.GetTile(x, y - 1).hasFloor()
                        and self.GetTile(x + 1, y).hasFloor()
                    ):
...
class Player:
    ...
    def LoadData(self, path: str):
        with open(path, "r") as file:
            ...
            self.currentMap = Map(
                self.level, self.playerSeed, 10, bandageCount=self.AmountBandages()
            )
    ...
    def getCameraOffset(self) -> tuple[float, float]:
        return (
            -(self.x * TILE_SIZE - SCREEN_WIDTH / 2),
            -(self.y * TILE_SIZE - SCREEN_HEIGHT / 2),
        )
    ...
    def Heal(self):
        # Add 10% health
        self.health = min(self.health + self.maxHealth * 0.1, self.maxHealth)
    ...
    def Interact(self) -> bool:
        ...
        for tile in [
            (
                self.currentMap.GetTile(math.floor(self.x - 1), math.floor(self.y - 1)),
                -1,
                -1,
            ),
            (
                self.currentMap.GetTile(math.floor(self.x - 1), math.floor(self.y)),
                -1,
                0,
            ),
            (
            ...
            (
                self.currentMap.GetTile(math.floor(self.x), math.floor(self.y - 1)),
                0,
                -1,
            ),
            (self.currentMap.GetTile(math.floor(self.x), math.floor(self.y)), 0, 0),
            (self.currentMap.GetTile(math.floor(self.x), math.floor(self.y + 1)), 0, 1),
            (
                self.currentMap.GetTile(math.floor(self.x + 1), math.floor(self.y - 1)),
                1,
                -1,
            ),
            (self.currentMap.GetTile(math.floor(self.x + 1), math.floor(self.y)), 1, 0),
            (
                self.currentMap.GetTile(math.floor(self.x + 1), math.floor(self.y + 1)),
                1,
                1,
            ...
            if tile[0] == Tile.CHEST and not self.currentMap.chestState:
                ...
                self.drops.append(
                    (
                        self.x,
                        self.y,
                        self.currentMap.mapSeed.GetRandom().choice(
                            ["redpotion", "greenpotion", "yellowpotion", "bluepotion"]
                        ),
                    )
                )
                ...
                self.Heal()
                self.currentMap.map[math.floor(self.x + tile[1])][
                    math.floor(self.y + tile[2])
                ].type = Tile.FLOOR
                self.currentMap.GenerateRenderMap()
    ...
    def Collide(self, x: float, y: float) -> bool:
        return self.currentMap.GetTile(
            math.floor(x + 1.5 / RAW_TILE_SIZE), math.floor(y + 7.5 / RAW_TILE_SIZE)
        ) in (
            [Tile.EMPTY, Tile.CHEST] if not self.currentMap.chestState else [Tile.EMPTY]
        )
    ...
    def Reset(self):
        ...
        self.drops = []
        self.level = 0
        self.errorFade = self.nextMapFade = self.hasMapOpen = self.facing = (
            self.dead
        ) = False
...
class Menu:
    ...
    def getRenderText(self) -> list[tuple[str, bool]]:
        if self.current == "Main":
            return [
                ("New Game", self.cursor == 0),
                ("Load Game", self.cursor == 1),
                ("Save Game", self.cursor == 2),
                ("Exit", self.cursor == 3),
            ]
        else:
            return [
                ("Game 1", self.cursor == 0),
                ("Game 2", self.cursor == 1),
                ("Game 3", self.cursor == 2),
            ]
