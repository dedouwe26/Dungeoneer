    ...
    def update(self, deltaTime: float):
        ...
        if not self.player.dead:
            for enemy in self.player.currentMap.enemies:
                if enemy.Update(self.player.x - 0.5, self.player.y - 0.5, deltaTime):
        ...
        if self.player.hasMapOpen and not self.player.dead:
            for tile in self.player.currentMap.GenerateMapRenderMap(
                self.player.x, self.player.y
            ):
                self.gameDisplay.blit(
                    mapTileset,
                    (
                        self.mapOffset[0] + tile[1] * MAP_TILE_SIZE,
                        self.mapOffset[1] + tile[2] * MAP_TILE_SIZE,
                    ),
                    mapTileRects[tile[0]],
                )
            ...
            for tile in self.player.currentMap.renderMap:
                self.gameDisplay.blit(
                    tileset,
                    (offset[0] + tile[1] * TILE_SIZE, offset[1] + tile[2] * TILE_SIZE),
                    tileRects[tile[0]],
                )

            for enemy in self.player.currentMap.enemies:
                self.gameDisplay.blit(
                    mirroredTileset if enemy.facing else tileset,
                    (offset[0] + enemy.x * TILE_SIZE, offset[1] + enemy.y * TILE_SIZE),
                    tileRects[enemy.variation]
                    if not enemy.facing
                    else pygame.Rect(
                        tileset.get_size()[0]
                        - tileRects[enemy.variation].left
                        - TILE_SIZE,
                        tileRects[enemy.variation].top,
                        tileRects[enemy.variation].width,
                        tileRects[enemy.variation].height,
                    ),
                )
            ...
            for drop in self.player.drops:
                self.gameDisplay.blit(
                    tileset,
                    (offset[0] + drop[0] * TILE_SIZE, offset[1] + drop[1] * TILE_SIZE),
                    tileRects[drop[2]],
                )
            ...
            for i in range(len(self.player.currentMap.projectiles)):
                ...
                direction = [
                    projectile[2].x - projectile[0],
                    projectile[2].y - projectile[1],
                ]
                ...
                if not distance < 0.5:
                    ...
                    cropped = pygame.Surface(
                        (tileRects["arrow"].width, tileRects["arrow"].height),
                        pygame.SRCALPHA,
                    )
                    cropped.blit(tileset, (0, 0), tileRects["arrow"])
                    self.gameDisplay.blit(
                        pygame.transform.rotate(cropped, angle),
                        (
                            offset[0] + projectile[0] * TILE_SIZE,
                            offset[1] + projectile[1] * TILE_SIZE,
                        ),
                    )
                else:
            ...
            sword = (
                ...
                else tileRects["ironsword"]
            )
            player = (
                (tileRects["armoredplayer"], tileRects["mirroredarmoredplayer"])
                if self.player.maxHealth >= 20
                else (tileRects["player"], tileRects["mirroredplayer"])
            )
                ...
                if self.isSwinging:
                    ...
                    self.gameDisplay.blit(
                        ...
                        (
                            ...
                            SCREEN_HEIGHT / 2 - TILE_SIZE * 1.5,
                        ),
                        sword,
                    )
                self.gameDisplay.blit(
                    mirroredTileset if self.player.facing else tileset,
                    (
                        SCREEN_WIDTH / 2 - TILE_SIZE / 2,
                        SCREEN_HEIGHT / 2 - TILE_SIZE / 2,
                    ),
                    player[1] if self.player.facing else player[0],
                )
            ...
            for i in range(len(self.crits)):
                ...
                self.gameDisplay.blit(
                    tileset,
                    (offset[0] + crit[0] * TILE_SIZE, offset[1] + crit[1] * TILE_SIZE),
                    tileRects["crit"],
                )
            ...
            self.gameDisplay.blit(
                tileset,
                (0, SCREEN_HEIGHT - TILE_SIZE / 2),
                tileRects["healthbackground"],
            )
            ...
            if percentageHealth >= 1:
                self.gameDisplay.blit(
                    tileset, (0, SCREEN_HEIGHT - TILE_SIZE / 2), tileRects["fullhealth"]
                )
            else:
                self.gameDisplay.blit(
                    tileset,
                    (
                        (percentageHealth * 46 - 46) * WORLD_SIZE,
            ...
            text = self.font.render(
                str(self.player.critchange * 100) + "%", False, (0, 39, 94)
            )
            rect = text.get_rect()
            ...
            # enemies left
            text = self.font.render(
                str(len(self.player.currentMap.enemies)), False, (240, 240, 221)
            )
            rect = text.get_rect()
            rect.top = 0
            rect.centerx = SCREEN_WIDTH / 2
            ...
            if self.player.nextMapFade or self.player.errorFade:
                ...
                if not self.downFade:
                    self.fadeLvl = min(
                        (255 if self.player.nextMapFade else 128),
                        self.fadeLvl
                        + deltaTime * (90 if self.player.errorFade else 80),
                    )
                    self.downFade = not (
                        self.fadeLvl < (255 if self.player.nextMapFade else 70)
                    )
                else:
                    self.fadeLvl = max(
                        0,
                        self.fadeLvl
                        - deltaTime * (50 if self.player.errorFade else 60),
                    )
        ...
        if self.player.dead:
            ...
            text = self.font.render(
                "You've reached level " + str(self.player.level), False, (204, 170, 68)
            )
    ...
    def onEvent(self, event: pygame.event.Event):
        if event.type == KEYDOWN:
            if event.dict["key"] == K_w if PC else K_UP:
                ...
                if self.player.hasMapOpen:
                    self.mapOffset = [
                        -(round(self.player.x) * MAP_TILE_SIZE) + SCREEN_WIDTH / 2,
                        -(round(self.player.y) * MAP_TILE_SIZE) + SCREEN_HEIGHT / 2,
                    ]
